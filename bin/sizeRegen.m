...
% coolant = 'Water';
% 
% inlet_temperature = 273.16;
% inlet_pressure = 300 * u.PSI2PA; % [Pa]
% T_l_melt = 273;
% 
...
%         hydraulic_D = 4 * h_c * w_c / (2 * (h_c + w_c)); % [m]
%         wall_area = w_c * length; % [m^2]
%         A = w_c * h_c; % channel area [m^2]
%    
...
%         k_w = 200; % Thermal Conductivity of Wall [W/m-K]
%         E = 70E9; % in Pa
%         CTE = 27E-6; % in 1/K
%         nu = 0.3; 
% 
...
%         mu_lw = py.CoolProp.CoolProp.PropsSI('V','T', T_wl, 'P', P_l, coolant); % viscosity of coolant along wall [Pa-s]
...
%         Nu_l = 0.027 * (Re_l ^ (4/5)) * (Pr_l ^ (1/3)) * (mu_lb / mu_lw) ^ 0.14; % nusselt number [] ALEX CITE SOURCE 
...
%             mu_lw = py.CoolProp.CoolProp.PropsSI('V','T', T_wl(i), 'P', P_l(i), coolant); % viscosity of coolant along wall [Pa-s]
...
title('Regenerative Cooling Thermals')
xlabel('Location [mm]')
