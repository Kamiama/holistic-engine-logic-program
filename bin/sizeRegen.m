...
for i = 1:points % where i is the position along the chamber (1 = injector, end = nozzle)
    ...
    while ~(converged) && counter < 250
        ...
        m_fin = sqrt(h_l(i) * P_fin / (k_w * A_c_fin(i))); % Fin m
        fin_q(i) = sqrt(h_l(i) * P_fin * k_w * A_c_fin(i)) * (T_base - T_l(i)) * tanh(m_fin * h_c_x(i)); % Fin heat transfer rate
        ...
        qdot_l(i) = h_l(i) * (T_wl(i) - T_l(i)) + fin_q(i); % liquid convective heat flux [W/m^2] (Heister EQ 6.29).
        ...
        if abs(qdot_g(i) - qdot_l(i)) > qdot_tolerance % check for tolerance
            ...
            if i < points
                ...
                % Calculate Strains
                epsilon_lc(i) = CTE*(T_wl(i)-T_l(i));
                epsilon_ll(i) = CTE*(T_wg(i)-T_l(i));  
                epsilon_tp(i) = ( ((P_l(i)-P_g(i))/2).*((w_c_x(i)./t_w_x(i)).^2) )  /E;
                epsilon_tt(i) = (E*CTE*qdot_g(i)*t_w_x(i))/(2*(1-nu)*k_w)  /E;
                epsilon_t(i) = (( ((P_l(i)-P_g(i))/2).*((w_c_x(i)./t_w_x(i)).^2) ) + (E*CTE*qdot_g(i)*t_w_x(i))/(2*(1-nu)*k_w)) / E; % tangential stress
...
set(gca, 'Ycolor', 'k')
axis equal;
legend('Von Mises Stress (Lands)','Von Mises Stress (Channels)', 'Chamber Contour','Location','best')
grid on
subplot(2,2,3)
hold on 
set(gca, 'FontName', 'Times New Roman')
