...
% parse & convert input properties
    ...
    % performance properties
        F = xlsread('input.xlsx','Engine', 'C8');                    % engine thrust [lbf]
        P_c = xlsread('input.xlsx', 'Engine', 'C9');                 % chamber pressure [psi]
        P_a = xlsread('input.xlsx', 'Engine', 'C11');                % ambient pressure [psi]
        P_e = xlsread('input.xlsx', 'Engine', 'C10');                 % exit pressure [psi] (equals P_a for optimal expansion)
        eff_c_star = xlsread('input.xlsx', 'Engine', 'C12');         % c* efficiency
        eff_c_f = xlsread('input.xlsx', 'Engine', 'C13');            % cF efficiency
    
    % fuel properties
        [~, txt] = xlsread('input.xlsx', 'Engine', 'H10');
        fuel = string(txt);                                          % fuel formula for NASA CEA
        fuel_weight = xlsread('input.xlsx','Engine', 'H13');         % fuel weights, does nothing now
        [~, txt] = xlsread('input.xlsx', 'Engine', 'H11');
        oxidizer = string(txt);                                      % oxidizer formula for NASA CEA
        fuel_temp = xlsread('input.xlsx','Engine', 'H17');           % inlet fuel temperature [K]
        oxidizer_temp = xlsread('input.xlsx','Engine', 'H18');       % inlet oxidizer temperature [K]
        OF = xlsread('input.xlsx', 'Engine', 'H14');                 % O/F ratio
        time_burn = xlsread('input.xlsx', 'Engine', 'C14');          % burn time [sec]
    ...
    % geometry properties
        [~, txt] = xlsread('input.xlsx', 'Engine', 'M10');
        geometry_type = string(txt);
        sizing_method = xlsread('input.xlsx', 'Engine', 'M11');       %sizing method
        bell_pct = xlsread('input.xlsx', 'Engine', 'M21');            % percent of bell nozzle
        conical_half_angle = xlsread('input.xlsx', 'Engine', 'M26');  % conical half angle [deg]
        L_crude_throat = xlsread('input.xlsx', 'Engine', 'M28');      % length of straight throat section [in]

        theta_i = xlsread('input.xlsx', 'Engine', 'M22');             % nozzle expansion (initial) angle [degrees]
        theta_e = xlsread('input.xlsx', 'Engine', 'M23');             % nozzle exit angle [degrees] 
        ...
        D_t = xlsread('input.xlsx', 'Engine', 'M19');
        L_star = xlsread('input.xlsx', 'Engine', 'M15');              % L*, characteristic combustion length [in]
        conv_angle = xlsread('input.xlsx', 'Engine', 'M16');          % convergence 
        con_ratio = xlsread('input.xlsx', 'Engine', 'M17');           % contraction ratio
        D_c = xlsread('input.xlsx', 'Engine', 'M18');                 % chamber diameter [in]
    
    % injector properties
        [~, txt] = xlsread('input.xlsx', 'Injector', 'C8');
    ...
    % parse tooling and stock sizes
        bar_size = xlsread('input.xlsx', 'Engine', 'M28'); % bar stock diameter [in]
